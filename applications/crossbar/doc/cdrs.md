...
`fax_bad_rows` | Number of rows that failed to transfer | `integer()` |   | `false` |  
`fax_ecm_used` | Was ECM (error correction mode) used on the fax | `boolean()` |   | `false` |  
`fax_result_code` | Media Server's result code of the transmission | `integer()` |   | `false` |  
`fax_result_text` | Error String, if any, or 'OK' if successful | `string()` |   | `false` |  
`fax_success` | Whether the fax was considered a success or not | `boolean()` |   | `false` |  
`fax_total_pages` | Number of pages in the fax | `integer()` |   | `false` |  
`fax_transfer_rate` | Baud of the fax transfer | `integer()` |   | `false` |  
`fax_transferred_pages` | Number of pages transferred | `integer()` |   | `false` |  
...
`interaction_key` |  | `string()` |   | `false` |  
`interaction_time` |  | `integer()` |   | `false` |  
...
`msg_id` | KAZOO specific key (Msg-Id) assigned to each payload it publishes | `integer()` |   | `false` |  
